...
def process_KiTS_image(item):
    try:
        ...
        if len(rtp) > 0:
            im_dir = os.path.join(dset_info[item['subdset']]["image_root_dir"], file_name, "imaging.nii.gz")
            label_dir = os.path.join(dset_info[item['subdset']]["label_root_dir"], file_name, "segmentation.nii.gz")
