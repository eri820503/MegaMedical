...
def process_T1mix_image(item):
    try:
        ...
        if len(rtp) > 0:
            # Skull-stripped
            vol_im_dir = os.path.join(dset_info[item['subdset']]['image_root_dir'], file_name)
            # Original volume
            norm_im_dir = os.path.join(dset_info[item['subdset']]['image_root_dir'].replace("vols", "origs"), file_name.replace("norm", "orig"))
            # Segmentation
            label_dir = os.path.join(dset_info[item['subdset']]['label_root_dir'], file_name.replace("norm", "aseg"))
