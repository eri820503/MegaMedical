...
def process_SegTHOR_image(item):
    try:
        ...
        if len(rtp) > 0:
            im_dir = os.path.join(dset_info[item['subdset']]['image_root_dir'], file_name, f"{file_name}.nii.gz")
            label_dir = os.path.join(dset_info[item['subdset']]['label_root_dir'], file_name, "GT.nii.gz")
