...
class SegTHOR:
    ...
    def proc_func(self,
        ...
        subj_dict, res_dict = proc.process_image_list(process_SegTHOR_image,
                                                      proc_dir,
                                                      image_list,
                                                      parallelize,
                                                      pps_function,
...
global process_SegTHOR_image
def process_SegTHOR_image(item):
    try:
        dset_info = item['dset_info']
        # template follows processed/resolution/dset/midslice/subset/modality/plane/subject
        if item['redo_processed'] or put.is_processed_check(item):
            im_dir = os.path.join(dset_info[item['subdset']]['image_root_dir'], item['image'], f"{item['image']}.nii.gz")
            label_dir = os.path.join(dset_info[item['subdset']]['label_root_dir'], item['image'], "GT.nii.gz")
