            ...
            for image in image_list:
                try:
                    if redo_processed or (len(glob.glob(os.path.join(processed_dir, "*", image))) == 0):
                        im_dir = os.path.join(self.dset_info["image_root_dir"], image, "aligned_norm.nii.gz")
                        label_dir = os.path.join(self.dset_info["label_root_dir"], image, "aligned_seg35.nii.gz")
