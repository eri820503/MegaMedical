...
class OASIS:
    ...
    def proc_func(self,
        ...
        subj_dict, res_dict = proc.process_image_list(process_OASIS_image,
                                                      proc_dir,
                                                      image_list,
                                                      parallelize,
                                                      pps_function,
...
global process_OASIS_image
def process_OASIS_image(item):
    try:
        dset_info = item['dset_info']
        # template follows processed/resolution/dset/midslice/subset/modality/plane/subject
        if item['redo_processed'] or is_processed_check(item):
            im_dir = os.path.join(dset_info[item['subdset']]["image_root_dir"], item['image'], "aligned_norm.nii.gz")
            label_dir = os.path.join(dset_info[item['subdset']]["label_root_dir"], item['image'], "aligned_seg35.nii.gz")
