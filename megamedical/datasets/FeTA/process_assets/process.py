...
class FeTA:
    ...
    def proc_func(self,
        ...
        subj_dict, res_dict = proc.process_image_list(process_FeTA_image,
                                                      proc_dir,
                                                      task,
                                                      image_list,
                                                      parallelize,
...
global process_FeTA_image
def process_FeTA_image(item):
    try:
        dset_info = item['dset_info']
        # template follows processed/resolution/dset/midslice/subset/modality/plane/subject
        rtp = item["resolutions"] if item['redo_processed'] else put.check_proc_res(item)
        if len(rtp) > 0:
            im_id = item['image'][4:7]

            im_dir = os.path.join(dset_info[item['subdset']]["image_root_dir"], item['image'], f"{im_id}_img.nii.gz")
            label_dir = os.path.join(dset_info[item['subdset']]["label_root_dir"], item['image'], f"{im_id}_seg.nii.gz")

            if dset_info['load_images']:
