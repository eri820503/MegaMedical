        ...
        for image in image_list:
            try:
                if redo_processed or (len(glob.glob(os.path.join(processed_dir, "*", image))) == 0):
                    im_dir = os.path.join(dset_info["image_root_dir"], image, f"{image}.nii.gz")
                    label_dir = os.path.join(dset_info["label_root_dir"], image, "GT.nii.gz")
